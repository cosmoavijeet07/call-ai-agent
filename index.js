...
const VOICE = "sage"; // The voice for AI responses
const PORT = process.env.PORT || 5050; // Set the port for the server (from environment or default to 5050)
const MAKE_WEBHOOK_URL = "https://hook.us2.make.com/7i1s9aqrhruvcbj1ijorwmmbk43bs38k"; // URL to Make.com webhook
...
fastify.all("/incoming-call", async (request, reply) => {
    ...
    let firstMessage = "Hello, welcome to Opticlaim. How can I assist you today?"; // Default first message
...
fastify.all("/outgoing-call-twiml", async (request, reply) => {
    ...
    const callerNumber = request.query.callerNumber || "Unknown";
...
fastify.register(async (fastify) => {
    fastify.get("/media-stream", { websocket: true }, (connection, req) => {
        ...
        console.log("Caller Number:", session.callerNumber);
                ...
                session: {
                    ...
                    tools: [
                        // Define the tools (functions) the AI can use
                        {
                            ...
                            description: "Get answers to customer questions or quaries about insurance policies and claims",
                            parameters: {
                                type: "object",
                                properties: {
                        ...
                        },
                        {
                            type: "function",
                            name: "file_claim",
                            description: "File an insurance claim for a customer",
                            parameters: {
                                type: "object",
                                properties: {
                                    policy_number: { type: "string" },
                                    incident_details: { type: "string" },
                                },
                                required: ["policy_number", "incident_details"],
                            },
                        },
                    ],
        ...
        openAiWs.on("message", async (data) => {
            try {
                ...
                if (response.type === "response.function_call_arguments.done") {
                    ...
                    } else if (functionName === "file_claim") {
                        // If the file _claim function is called
                        const incident = args.incident; // Get the address
                        const policy = args.policy; // Get the address
                        try {
                            const webhookResponse = await sendToWebhook({
                                route: "4", // Route 4 for booking a tow
                                data1: session.callerNumber,
                                data2: incident,
                                data3: policy, // Send the address to the webhook
                            });
                            ...
                            // Parse the webhook response
                            const parsedResponse = JSON.parse(webhookResponse);
                            const bookingMessage = parsedResponse.message || "I'm sorry, I couldn't file the claim at this time.";
                            ...
                            openAiWs.send(
                                JSON.stringify({
                                    ...
                                    response: {
                                        modalities: ["text", "audio"],
                                        instructions: `Inform the user about the claim file status: ${bookingMessage}. Be concise and friendly.`,
                                    },
                                }),
                            );
                        } catch (error) {
        ...
        connection.on("close", async () => {
            ...
            await sendToWebhook({
                ...
                data3: "empty", // Send the transcript to the webhook
            });
